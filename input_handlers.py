...
CURSOR_Y_KEYS = {
    tcod.event.K_UP: -1,
    tcod.event.K_DOWN: -1,
    tcod.event.K_PAGEUP: -1,
    tcod.event.K_PAGEDOWN: -1,
}
...
If a handler is return then it will become the active handler for future events
...
class EventHandler(BaseEventHandler):
    ...
    def handle_events(self, event: tcod.event.Event) -> BaseEventHandler:
        """
        Handle events for input handles with an engine
        """
...
class GameOverEventHandler(EventHandler):
    ...
    def on_quit(self) -> None:
        ...
        if os.path.exists('savegame.sav'):
            os.remove('savegame.sav')  # delete current save
...
class AskUserEventHandler(EventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[Action, BaseEventHandler]:
...
class InventoryEventHandler(AskUserEventHandler):
    ...
    def on_item_selected(self, item: Item) -> Optional[ActionOrHandlern]:
...
class SelectIndexHandler(AskUserEventHandler):
    ...
    def ev_keydown(self, event: tcod.event.KeyDown) -> Optional[ActionOrHandler]:
        ...
        key = event.sym
        if key in MOVE_KEYS:
            modifier = 1  # holding modifier keys will speed up key movements
